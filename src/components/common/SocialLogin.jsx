import React from 'react';
import useAuth from '../../hooks/useAuth';
import { useLocation, useNavigate } from 'react-router';

const SocialLogin = () => {

    const { signInWithGoogle } = useAuth();
    const location = useLocation();
    const navigate = useNavigate();

    const handleLoginWithGoogle = () => {
        signInWithGoogle()
            .then(res => {
                console.log('after login with google', res);
                navigate(location.state || '/');
            })
            .catch(err => {
                console.log('error during login with google', err)
            });
    }

    return (
        <div className='w-full'>

            <p className='text-center text-2xl font-semibold my-3'>Or</p>

            {/* Google */}
            <button onClick={handleLoginWithGoogle} className="btn bg-white text-black  w-full border border-rose-400 hover:bg-rose-500 hover:text-white">
                <svg aria-label="Google logo" width="20" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><g><path d="m0 0H512V512H0" fill="#fff"></path><path fill="#34a853" d="M153 292c30 82 118 95 171 60h62v48A192 192 0 0190 341"></path><path fill="#4285f4" d="m386 400a140 175 0 0053-179H260v74h102q-7 37-38 57"></path><path fill="#fbbc02" d="m90 341a208 200 0 010-171l63 49q-12 37 0 73"></path><path fill="#ea4335" d="m153 219c22-69 116-109 179-50l55-54c-78-75-230-72-297 55"></path></g></svg>
                Login with Google
            </button>
        </div>
    );
};

export default SocialLogin;